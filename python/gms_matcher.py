...
class GmsMatcher:
    def __init__(self, descriptor, matcher):
        self.scale_ratios = [1.0, 1.0 / 2, 1.0 /
    ...
    def compute_matches(self, img1, img2):
        self.keypoints_image1, descriptors_image1 = self.descriptor.detectAndCompute(
            img1, np.array([]))
        self.keypoints_image2, descriptors_image2 = self.descriptor.detectAndCompute(
            img2, np.array([]))
        ...
        all_matches = self.matcher.match(
            descriptors_image1, descriptors_image2)
        self.normalize_points(self.keypoints_image1,
                              size1, self.normalized_points1)
        self.normalize_points(self.keypoints_image2,
                              size2, self.normalized_points2)

        self.matches_number = len(all_matches)
        self.convert_matches(all_matches, self.matches)
        # 把all_matches的匹配列表转换成(kp1,kp2)这样得到形式

        self.initialize_neighbours(
            self.grid_neighbor_left, self.grid_size_left)
        # 把所有grid_neighbor_left中的所有9*400中的9向量变成每个格子周围9个的下标
    ...
    def initialize_neighbours(self, neighbor, grid_size):
        for i in range(neighbor.shape[0]):  # 这里是400 即20*20 ;grid_size 20,20
            neighbor[i] = self.get_nb9(i, grid_size)  # neighbor中是每个格子周围九个格子的序号
    ...
    def set_scale(self, scale):
        self.grid_size_right.width = self.grid_size_left.width * \
            self.scale_ratios[scale]
        self.grid_size_right.height = self.grid_size_left.height * \
            self.scale_ratios[scale]
        ...
        self.initialize_neighbours(
            self.grid_neighbor_right, self.grid_size_right)

    def run(self, rotation_type):
        self.inlier_mask = [False for _ in range(self.matches_number)]
        # match_number is all matches，初始化所有匹配都是错的

        # Initialize motion statistics
        self.motion_statistics = np.zeros(
            (int(self.grid_number_left), int(self.grid_number_right)))  # scale=0 这时left.size = right=400
        self.match_pairs = [[0, 0] for _ in range(self.matches_number)]

        for GridType in range(1, 5):
            # 这里循环五次是为了在assign_match_pairs中多给边界点一些机会
            # 然后在进行验证匹配 会得到多一些的正确匹配
            self.motion_statistics = np.zeros(
                (int(self.grid_number_left), int(self.grid_number_right)))
            self.cell_pairs = [-1 for _ in range(self.grid_number_left)]
            self.number_of_points_per_cell_left = [
                0 for _ in range(self.grid_number_left)]

            self.assign_match_pairs(GridType)
            self.output_to_txt(self.motion_statistics, GridType)
    ...
    def output_to_txt(self, input_message, GridType):
        f = open("motion_statistics"+str(GridType)+".txt", "w")
        for line in input_message:
            print(str(line).replace('.', '')[1:-2], file=f)
        # print(type(input_message), input_message)

    def assign_match_pairs(self, grid_type):
        for i in range(self.matches_number):
            lp = self.normalized_points1[self.matches[i][0]]
            # matches里是matches_number个imgl,imgr对应的特征点
            # normalized_points1里是img中特征点坐标的个数个特征点坐标区间(0,1)
            rp = self.normalized_points2[self.matches[i][1]]
            lgidx = self.match_pairs[i][0] = self.get_grid_index_left(
                lp, grid_type)
    ...
    def verify_cell_pairs(self, rotation_type):
        ...
        for i in range(self.grid_number_left):
            if sum(self.motion_statistics[i]) == 0:
                # sum(self.motion_statistics[i])
                # self.number_of_points_per_cell_left[i]
                self.cell_pairs[i] = -1
                continue
            max_number = 0
            for j in range(int(self.grid_number_right)):
                ...
                if value[j] > max_number:
                    self.cell_pairs[i] = j  # cell_pairs维护一个网格之间最匹配的匹配对
                    max_number = value[j]

            idx_grid_rt = self.cell_pairs[i]  # 和imgl i格子最匹配的imgr j格子
            nb9_lt = self.grid_neighbor_left[i]  # 分别是两个格子周围九个格子[1 2 3 ...]的格式
        ...
        if drawing_type == DrawingType.ONLY_LINES:
            for i in range(len(self.gms_matches)):
                ...
                right = tuple(sum(x) for x in zip(
                    self.keypoints_image2[self.gms_matches[i].trainIdx].pt, (src1.shape[1], 0)))
                cv2.line(output, tuple(map(int, left)),
                         tuple(map(int, right)), (0, 255, 255))
